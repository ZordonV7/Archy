...
class Settings(BaseSettings):
    ...
    # Vercel URL, e.g. "https://archy.vercel.app,https://www.archy.vercel.app".
    allowed_origins: str = "*"
    ...
    secret_key: str = Field(default="dev-secret-change-me", alias="ARCHY_SECRET_KEY")
    # Frontend URL — used for OAuth callback redirects (where to send the user
    # after Google auth completes). Example: "https://archy.vercel.app"
    # Desktop default is fine for local dev.
    frontend_url: str = Field(default="http://localhost:5173", alias="ARCHY_FRONTEND_URL")
    ...
    require_auth: bool = Field(default=False, alias="ARCHY_REQUIRE_AUTH")

    # --- Storage ---
    # Desktop (default): SQLite at ~/.archy/archy.db
    # Web deployment: set ARCHY_DB_URL to a PostgreSQL connection string
    ...
    db_path: Path = Path("~/.archy/archy.db").expanduser()
    db_url: str = Field(default="", alias="ARCHY_DB_URL")
    ...
    # TTS model: 10 RPD — for Archy's voice (when frontend ships)
    ...
    google_token_path: Path = Path("~/.archy/google_tokens.json").expanduser()

...
or any managed Postgres). It mirrors ``archy/core/storage.py`` (SQLite)
...
Selection rules:
    ...
    - ``PostgresRepository`` is selected via the ``ARCHY_DB_URL`` env var
...
class PostgresRepository:
    ...
    corresponding method on ``archy.core.storage.Repository``. Callers
...
def get_repository(settings=None) -> PostgresRepository:
    ...
    the connection URL is read exclusively from the ``ARCHY_DB_URL``
    environment variable (a libpq connection string).

    Raises:
        RuntimeError: if ``ARCHY_DB_URL`` is not set.
    """
    global _repo
    ...
    with _repo_lock:
        if _repo is None:
            db_url = os.environ.get("ARCHY_DB_URL")
            if not db_url:
                raise RuntimeError(
                    "ARCHY_DB_URL env var not set — required to construct "

...
class EnergyEngine:
    ...
    def __init__(self, settings: Settings, repo: Repository):
        ...
        self._s = settings  # FIX: previously never stored — workday config was ignored
        self._repo = repo
    ...
    def current(self) -> EnergySnapshot:
        ...
        # Build actionable recommendations based on the density label + upcoming
        # deadlines — previously this just split the description string, which
        # produced nonsense like ["Schedule is empty", ""] for the user.
        recommendations: list[str] = []
        density_label = density_info.get("density_label", "")
        if display_score < 40:
            recommendations.append("Rest before starting complex tasks.")
        elif display_score < 60:
            recommendations.append("Prefer light tasks; avoid deep-focus work.")
        upcoming = density_info.get("upcoming_deadlines", [])
        if any(d.get("hours_left", 999) < 6 for d in upcoming):
            recommendations.append("Tackle the nearest deadline first.")
        if density_label in ("busy", "packed"):
            recommendations.append("Consider rescheduling lower-priority items.")
        if not recommendations:
            recommendations.append("You have room for focused work.")
    ...
    def get_schedule_density(self) -> dict:
        ...
        # FIX: previously `self._s` was never stored in __init__, so this always
        # fell back to 780 min even when the user had customized their workday.
        try:
            wh_start = self._parse_hhmm(self._s.workday_start)
            wh_end = self._parse_hhmm(self._s.workday_end)
            workday_minutes = (wh_end.hour - wh_start.hour) * 60 + (wh_end.minute - wh_start.minute)
        except Exception:
            workday_minutes = 780  # default 13h fallback

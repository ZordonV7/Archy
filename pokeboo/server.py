"""HTTP + WebSocket server for PoKeBoo.
...
from .pokebo.manager import PokeBo


class EventHub:
    """In-memory pub/sub for PokeBo events → WebSocket clients."""
...
def create_app(
    ...
    async def lifespan(app: FastAPI):
        ...
        if pokebo is None:
            ...
            pokebo = PokeBo(settings, repo, gemini)
        pokebo.subscribe(hub.publish)
        app.state.pokebo = pokebo
        app.state.repo = repo
        app.state.hub = hub
        logger.info(
            f"PoKeBoo server up on {settings.host}:{settings.port} ({settings.mode} mode)"
        )
        yield
        repo.close()

    app = FastAPI(title="PoKeBoo", version="0.2.0", lifespan=lifespan)
    ...
    app.add_middleware(
        ...
        session_cookie="pokeboo_session",
    ...
    async def ingest_text(req: IngestTextRequest) -> dict:
        task = await _pokebo().ingest_text(req.text)
    ...
    async def ingest_audio(file: UploadFile) -> dict:
        ...
        task = await _pokebo().ingest_audio(
    ...
    async def start_task(task_id: str) -> dict:
        try:
            t = await _pokebo().start_task(task_id)
        except KeyError:
            raise HTTPException(404, f"Task {task_id} not found")
        return t.model_dump(mode="json")
    ...
    async def cancel_task(task_id: str) -> dict:
        try:
            t = await _pokebo().cancel_task(task_id)
...
"""
    ...
    async def get_schedule() -> dict:
        return await _pokebo().replan()

    @app.get("/mood")
    async def get_mood() -> dict:
        return await _pokebo().current_mood()
    ...
    async def get_energy() -> dict:
        return await _pokebo().current_energy()
    ...
    async def deadline_notifications() -> dict:
        ...
        smart_notifications = pokebo._notification_manager.check_deadlines(tasks)
    ...
    async def learned_biases() -> dict:
        """Show what PoKeBo has learned about the user's estimation biases."""
        from .core.scheduler_trainer import SchedulerTrainer
        trainer = SchedulerTrainer(repo)
    ...
    async def productivity_profile() -> dict:
        ...
        return _pokebo()._productivity_model.get_profile_summary()

    @app.get("/productivity/profile/raw")
    async def productivity_profile_raw() -> dict:
        """Get the raw productivity profile data."""
        return _pokebo()._productivity_model.get_profile().model_dump()
    ...
    async def notification_strategy() -> dict:
        ...
        return _pokebo()._notification_manager.get_recommended_strategy()
    ...
    async def check_notifications() -> dict:
        ...
        tasks = repo.list_tasks()
        notifications = _pokebo()._notification_manager.check_deadlines(tasks)
        return {
    ...
    async def record_notification_action(notification_id: str, acted: bool = True) -> dict:
        ...
        _pokebo()._notification_manager.record_action(notification_id, acted)
    ...
    async def run_simulation() -> dict:
        return await _pokebo().simulate_failure()

    # v2.1: Daily briefing ("Good morning, here's your day")
    @app.get("/briefing")
    async def get_briefing() -> dict:
        return await _pokebo().generate_briefing()
    ...
    async def report_drift(event: DriftEvent) -> dict:
        return await _pokebo().report_drift(event)
    ...
    async def list_pending_permissions() -> dict:
        ...
        return {"permissions": _pokebo()._proposal_manager.list_pending()}
    ...
    async def approve_permission(permission_id: str) -> dict:
        ...
        pending = pokebo._proposal_manager.list_pending()
        ...
        result = await pokebo._proposal_manager.approve(permission_id, approved_by="user")
        return result
    ...
    async def deny_permission(permission_id: str) -> dict:
        ...
        pokebo = _pokebo()
        result = await pokebo._proposal_manager.deny(permission_id)
        return result
    ...
    async def google_auth_login(request: Request) -> RedirectResponse:
        ...
        # (e.g. "https://pokeboo-backend.onrender.com/auth/google/callback")
    ...
    async def generate_daily_brief() -> dict:
        ...
        return await _pokebo().generate_daily_brief()
...
# For `uvicorn pokeboo.server:app`
app = create_app()

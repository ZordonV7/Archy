"""Gemini TTS client — generates spoken audio for PokeBo's messages.

Uses `gemini-3.1-flash-tts-preview` (10 RPD, 3 RPM).
Very limited quota — only use for high-value messages (PokeBo's voice).
...
# PokeBo's voice preset — warm, friendly, slightly higher pitch
POKEBO_VOICE_STYLE = "Warm, friendly, slightly soft-spoken, gentle pace, encouraging tone"


class TTSClient:
    """Generates spoken audio for PokeBo's user-facing messages."""
    ...
    async def synthesize(
        self,
        text: str,
        voice_style: str = POKEBO_VOICE_STYLE,
    ) -> bytes:
        """Convert text to spoken audio bytes.

        Args:
            text: The text to speak (PokeBo's message)

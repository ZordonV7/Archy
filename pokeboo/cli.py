...
app = typer.Typer(no_args_is_help=True, help="PoKeBoo backend CLI.")
...
def ingest_text(text: str) -> None:
    ...
    async def _run():
        ...
        pokebo = PokeBo(s, repo, gemini)
        t = await pokebo.ingest_text(text)
...
def ingest_audio(path: Path) -> None:
    ...
    async def _run():
        ...
        t = await pokebo.ingest_audio(audio, path.name, mime)
...
@app.command()
def conflicts() -> None:
    """Show recent conflict events (PokeBo vs agents)."""
    repo = get_repository()
...
def drift(
    ...
    async def _run():
        ...
        result = await pokebo.report_drift(event)
...
def sync_calendar() -> None:
    ...
    async def _run():
        ...
        if not s.calendar_sync_enabled:
            typer.echo("Calendar sync is disabled. Set POKEBOO_CALENDAR_SYNC_ENABLED=true in .env")
            return
        repo = get_repository(s)
        gemini = get_gemini_client(s)
        pokebo = PokeBo(s, repo, gemini)
        # Trigger a replan, which will fire plan.ready → Calendar subscriber
        result = await pokebo.replan()
...
def daily_brief() -> None:
    ...
    async def _run():
        ...
        gemini = get_gemini_client(s)
        pokebo = PokeBo(s, repo, gemini)
        result = await pokebo.generate_daily_brief()
        if "error" in result:

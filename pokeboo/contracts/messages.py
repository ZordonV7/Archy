...
class AgentProposal(BaseModel):
    ...
    Every agent call produces one of these. PokeBo evaluates the
...
# --------------------------------------------------------------------------- #
# PokeBo's evaluation of a proposal
# --------------------------------------------------------------------------- #
...
class PokeBoEvaluation(BaseModel):
    """PokeBo's verdict on an agent proposal.

    The `relevancy_score` is computed by PokeBo (via the LLM) based on:
...
class ConflictResolution(StrEnum):
    """How a conflict between PokeBo and an agent ended up."""

    ACCEPTED = "accepted"           # agent's original proposal accepted
    REVISED = "revised"             # agent re-ran with PokeBo's constraints
    REJECTED = "rejected"           # PokeBo vetoed; fell back to rule-based core
    ESCALATED = "escalated"         # max rounds hit; user notified


class ConflictEvent(BaseModel):
    """Emitted when PokeBo disagrees with an agent. Persisted + published.

    The UI can later show these as "PokeBo disagreed with Planner because
...
# --------------------------------------------------------------------------- #
# PokeBo's user-facing output
# --------------------------------------------------------------------------- #


class UserFacingMessage(BaseModel):
    """PokeBo's interpreted output — the only natural language the user sees."""

    text: str
    mood_label: str  # the mood PokeBo is matching tone to

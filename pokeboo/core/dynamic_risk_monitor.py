...
risk trajectory. This module hooks into PokeBo's event system and recalculates
...
class DynamicRiskMonitor:
    """Monitors risk continuously by hooking into PokeBo's event system.

    Usage:
        monitor = DynamicRiskMonitor(settings, repo, risk_engine)
        pokebo.subscribe(monitor)
        ...
        try:
            ...
            if snapshot:
                ...
                if abs(new - old) >= 3 or event_type == "PlanReady":
                    ...
                    # The caller (PokeBo) will emit the event — we just return the data
                    # PokeBo checks _last_snapshot after each event
        except Exception:

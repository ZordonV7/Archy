...
- pokebo_evaluations (NEW — PokeBo's verdict on each proposal)
- conflict_events    (NEW — when PokeBo and an agent disagreed)
"""
...
class Repository:
    ...
    def append_evaluation(self, e: PokeBoEvaluation) -> None:
        with self._lock:
            self._conn.execute(
                """
                INSERT OR REPLACE INTO pokebo_evaluations (
...
def get_repository(settings: Settings | None = None) -> Repository:
    ...
    - If `POKEBOO_DB_URL` is set (web deployment): returns a `PostgresRepository`

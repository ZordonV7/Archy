"""PokeBo — the manager, supervisor, and interpreter.
...
Invariant: PokeBo is the ONLY entity that produces natural language for
...
class PokeBo:
    ...
    async def ingest_audio(self, audio_bytes: bytes, filename: str, mime: str) -> Task:
        logger.info(f"PokeBo ingesting audio: {filename} ({len(audio_bytes)} bytes)")
    ...
    async def ingest_text(self, text: str) -> Task:
        logger.info(f"PokeBo ingesting text ({len(text)} chars)")
    ...
    async def replan(self) -> dict:
        ...
        # Synthesize PokeBo's user-facing message
        ...
        await self._emit("PokeBoMessage", ufm.model_dump(mode="json"))
        ...
        if assessments:
            ...
            # If any task is high-risk, PokeBo mentions it
            ...
            if high_risk:
                ...
                await self._emit("PokeBoMessage", {"text": risk_msg, "mood_label": label, "type": "risk_alert"})
    ...
    async def complete_task(self, task_id: str) -> Task:
        ...
        await self._emit("PokeBoMessage", {"text": msg, "mood_label": label})
    ...
    async def report_drift(self, event: DriftEvent) -> dict:
        ...
        await self._emit("PokeBoMessage", {"text": msg, "mood_label": label})
        await self.replan()
        return {"mood": {"score": score, "label": label}, "pokebo_message": msg}
    ...
    async def get_insights(self) -> dict:
        ...
        Shows what PoKeBo has learned about the user's patterns.
        """
    ...
    async def _generate_fresh_briefing(self) -> dict:
        ...
        # Generate PokeBo's intro using Gemma (least resource hungry)
        ...
        brief_prompt = (
            ...
            f"Respond as PokeBo. ONE sentence. Max 15 words. Friendly greeting for the day.\n"
    ...
    async def generate_daily_brief(self) -> dict:
        ...
        if not self.settings.daily_brief_enabled:
            return {
                "error": "Daily brief is disabled. Set POKEBOO_DAILY_BRIEF_ENABLED=true."
            }

        from ..integrations.auth import GoogleAuth
        auth = GoogleAuth(self.settings)
        if not auth.is_authenticated():
            return {"error": "Not authenticated. Run `pokeboo auth` first."}
    ...
    async def _execute_daily_brief(self, parameters: dict) -> dict:
        ...
        # Compose PokeBo's friendly intro (uses the LLM)
        ...
        title = f"PoKeBoo Daily Brief - {utcnow().strftime('%B %d, %Y')}"
        sections = [
            {"heading": "From PokeBo", "body": intro},
        ...
        while True:
            ...
            if round_num >= self.settings.max_negotiation_rounds:
                ...
                conflict = ConflictEvent(
                    ...
                    final_outcome=(
                        f"PokeBo vetoed after {round_num} rounds; "
            ...
            logger.info(
                f"Round {round_num}: PokeBo rejected {proposal.agent_name} "
            ...
            conflict = ConflictEvent(
                ...
                final_outcome="Agent revised proposal per PokeBo's constraints.",
            )
            self.repo.append_conflict(conflict)
            await self._emit("ConflictEvent", conflict.model_dump(mode="json"))

    async def _evaluate_proposal(self, proposal: AgentProposal) -> PokeBoEvaluation:
